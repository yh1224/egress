...
static void help()
{
  fprintf(stderr, "pkt-correct\n");
  fprintf(stderr, "\tInput packets from stdin and correct checksum.\n\n");
  fprintf(stderr, "EXAMPLE:\n");
  fprintf(stderr, "\t$ pkt-recv -i eth0 | pkt-correct | pkt-send -i eth1\n");
  fprintf(stderr, "\t$ cat packet.txt | pkt-correct | pkt-analyze\n\n");
...
int main(int argc, char *argv[])
{
  ...
  while (!terminated) {
    ...
    size = pkt_correct_ethernet(buffer, size);
    if (size < 1)
      continue;
