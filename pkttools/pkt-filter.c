...
static void help()
{
  fprintf(stderr, "pkt-filter\n");
  fprintf(stderr, "\tInput packets from stdin and filter packets.\n\n");
  fprintf(stderr, "EXAMPLE:\n");
  fprintf(stderr, "\t$ pkt-recv -i eth0 | pkt-filter | pkt-send -i eth1\n");
  fprintf(stderr, "\t$ cat packet.txt | pkt-filter | pkt-analyze\n\n");
...
int main(int argc, char *argv[])
{
  ...
  while (!terminated) {
    ...
    if (pkt_filter_ethernet(buffer, size) < 1)
      continue;
