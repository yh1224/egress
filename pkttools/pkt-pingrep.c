...
static void help()
{
  fprintf(stderr, "pkt-pingrep\n");
  fprintf(stderr, "\tInput packets from stdin and make reply.\n\n");
  fprintf(stderr, "EXAMPLE:\n");
  fprintf(stderr, "\t$ pkt-recv -i eth0 | pkt-pingrep | pkt-send -i eth0\n");
  fprintf(stderr, "\t$ pkt-recv -i eth0 | pkt-filter | pkt-pingrep | pkt-send -i eth0\n\n");
...
int main(int argc, char *argv[])
{
  ...
  while (!terminated) {
    ...
    size = pkt_pingrep_ethernet(buffer, size);
    if (size < 1)
      continue;
